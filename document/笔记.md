...
* 基于角色的权限控制
  ...
  * 第三步：给角色配置能访问的资源（**这一步采用切面拦截，使用的是注解**）
    ...
    1. `PreAuthorize`在方法调用前进行权限检查；（常用）
       ...
       如果在getAuthorities方法中权限列表存放时没有以ROLE开头，则需要使用hasAuthority
       @PreAuthorize("hasAuthority('saler')") // 需要有saler权限
       @PreAuthorize("hasAnyAuthority('admin'， 'manager')") //需要有admin或者manager权限
       ```
